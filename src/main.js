// 样式表已改为 index.html 中 <link rel="stylesheet"> 渲染阻塞式加载（见 head 注释），
// 不再通过 JS 模块注入，避免 CSS 就绪前内容以默认样式绘制造成闪变。
...
function hideAppLoader() {
    ...
    // 双 rAF：确保加载器至少完整绘制过一帧再开始淡出，
    // 否则快速刷新时它会在首帧前就被置为隐藏，导致底色闪变。
    // 后台标签页中 rAF 会被节流甚至不触发，用定时器兜底，避免加载器滞留。
...
function calculate() {
    ...
    // 总价值 = 整个付款周期的价值（续费价格 × 汇率），与日期无关，
    // 因此放在日期校验的提前返回之前，任何输入状态下都保持更新。
    els.totalValue.textContent = `¥ ${totalCNY.toFixed(2)}`;
        ...
        try {
            ...
            const canvas = await htmlToImage.toCanvas(node, {
                ...
                filter: (element) => {
                    ...
                    return !['themeToggle', 'btnContainer', 'footerWrap', 'headerWrap'].includes(element.id);
                },
                style: {
                    transform: 'scale(1)',
                }
